...
| **REQ-05-14** | Workspace destination persistence via `last_workspace_destination` | `src/state/session.rs`, `src/views/workspace.rs` | Tab tracking ignoring Settings | `state::session::tests::test_last_workspace_destination_tracks_non_settings` | **PASSED** |
...
| **REQ-05-18** | Real package management deletion flags | `src/backend/client.rs`, `src/views/workspace.rs` | `--cascade` and `--remove-config` flags | Verified in `ShellyClient::remove_package` | **PASSED** |
...
| **REQ-05-20** | Strict package build checking (`PKGBUILD-gpui`) | `PKGBUILD-gpui` | Removal of `\|\| true` from `check()` | Verified strict `cargo test --release` in recipe | **PASSED** |
| **REQ-05-21** | Interactive news feed with URL opening | `src/views/news.rs` | `cx.open_url` on news card click | Verified hover feedback and `AppIcon::ExternalUrl` | **PASSED** |
