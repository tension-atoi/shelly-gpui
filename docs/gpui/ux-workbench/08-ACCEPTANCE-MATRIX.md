...
| Requirement | Specification | Implementation | Verification Method | Status |
| :--- | :--- | :--- | :--- | :--- |
| **Provenance-Backed Tier 1 (ALPM/AUR)** | For ALPM & AUR, inspect `/var/lib/pacman/local/<pkg>-<version>/files`, find owned `usr/share/applications/*.desktop`, extract `Icon=`, and resolve on disk. Uninstalled packages or packages without owned desktop entries honestly return Tier 2 Symbolic (`SourceAlpm`, `SourceAur`). Zero fake logos / zero guessing | `components::package_identity` with `probe_alpm_or_aur_icon`, `parse_desktop_icon_field`, `resolve_icon_name`; strict provenance check | Unit tests `test_alpm_uninstalled_never_checks_disk`, `test_resolve_identity_3_tier_chain`; live Wayland verification | **PASSED** |
| **Zero Sync Disk I/O on Hot-Path** | `render_avatar` and `render_inline_glyph` must never do synchronous disk I/O (`stat`, `exists`, `is_file`) during row rendering | `IdentityCache` static store (`OnceLock<RwLock<HashMap<PackageKey, ResolvedIdentity>>>`): cache hits return in $O(1)$ memory; cache misses return Tier 2 Symbolic in $O(1)$ immediately and schedule background async prefetch | Unit test `test_identity_cache_zero_io_and_preload`; proactive `PackageIdentity::preload` in `workspace.rs` | **PASSED** |
...
| **Zero Deadcode & 106/106 Tests** | `#![deny(dead_code)]` with zero warnings; 106 unit tests passing in release locked profile | Strict YAGNI, 106 unit tests passing in release locked profile | `cargo test --release --locked` exits 0 (106 passed), `cargo clippy` exits 0 | **PASSED** |
| **Native PKGBUILD Installation** | Arch package built and installed matching commit SHA | Packaged via `makepkg -C -c -f`, installed via `pacman -U` | `pacman -Q shelly-gpui-git` confirmed `r4699.ga172c43e-1` | **PASSED** |
